...
class FragmentTag extends TagLib
{
    ...
    protected $tags = array(
        ...
        'classs'=>['attr'=>'field,num,order,sqlstr,cache','level'=>3],
    ...
    //栏目查询标签
    public function tagClasss($tag,$content) {
        $result= !empty($tag['result'])?$tag['result']: 'classs';
        $map.=($tag['sqlstr'])?" {$tag['sqlstr']} ":"1=1";
        $sql ="db('catagory')->";
        $sql.=($tag['field'])?"field('class_id,pid,filepath,filename,class_name,jumpurl,{$tag['field']}')->":"field('class_id,pid,class_name,jumpurl,filepath,filename')->";
        $sql.="where(\"{$map}\")->";
   ...
   //栏目查询标签
	public function tagClass($tag,$content) {
		...
		$map.=($tag['sqlstr'])?" {$tag['sqlstr']} ":"1=1";
        $sql ="db('catagory')->";
		$sql.=($tag['field'])?"field('class_id,subtitle,pid,filepath,filename,class_name,jumpurl,{$tag['field']}')->":"field('class_id,pid,subtitle,class_name,jumpurl,filepath,filename')->";
		$sql.="where(\"{$map}\")->";
        $sql.=($tag['order'])?"order('{$tag['order']}')->":"order('sortid asc')->";
		$sql.=($tag['group'])?"group('{$tag['group']}')->":"";
        $sql.=($tag['num'])?"limit({$tag['num']})->":"";
   ...
   public function tagList($tag,$content) {
        ...
        $sql ="db('content')->";
		if($tag['extend']){
			$sql.=($tag['field'])?"field('a.content_id,a.title,a.pic,a.views,a.jumpurl,a.description,a.create_time,a.class_id,{$tag['field']}')->":"field('a.content_id,a.title,a.pic,a.views,a.description,a.jumpurl,a.create_time')->";
		}else{
			$sql.=($tag['field'])?"field('content_id,title,pic,description,jumpurl,views,create_time,class_id,{$tag['field']}')->":"field('content_id,title,pic,views,description,jumpurl,create_time,class_id')->";
		}
		$sql.=($tag['extend'])?"alias('a')->":"";
		$sql.=($tag['extend'])?"join('{$tag['extend']} b','a.content_id=b.content_id')->":"";
		$sql.="where(\"{$map}\")->";
