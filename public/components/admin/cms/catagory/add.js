Vue.component('Add', {
	template: `
		<el-dialog title="添加" width="580px" class="icon-dialog" :visible.sync="show" @open="open" :before-close="closeForm" append-to-body>
	...
	methods: {
		open(){
			if(this.info && JSON.stringify(this.info) !== '{}'){
				this.form = this.info
				this.form.pid = this.form.class_id
			}else{
				this.form = {
					type:1,
					status:1,
		...
		submit(){
			this.$refs['form'].validate(valid => {
				if(valid) {
					this.loading = true
					axios.post(base_url + '/Cms.Catagory/add',this.form).then(res => {
